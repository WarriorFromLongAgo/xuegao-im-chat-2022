package com.xuegao.im.client;

import com.xuegao.im.client.handler.NettyClientHandlerInitializer;
...
public class NettyClient implements CommandLineRunner, DisposableBean {
     ...
     */
    private AtomicReference<Channel> atomicReferenceChannel;
    ...
    public void send(Invocation invocation) {
        if (atomicReferenceChannel == null || this.atomicReferenceChannel.get() == null) {
            logger.error("[send][连接不存在]");
            return;
        }
        Channel channel = this.atomicReferenceChannel.get();
        if (!channel.isActive()) {
    ...
    public void run(String... args) throws Exception {
        ...
        bootstrap.connect().addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                ...
                atomicReferenceChannel = new AtomicReference<>();
                atomicReferenceChannel.set(channel);
